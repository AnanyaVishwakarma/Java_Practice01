...
without any particular order, and no repeated values(Wikipedia).{1,2,3} is an example of a set, but {1,2,2} is not a set. 
Today you will learn how to use sets in java by solving this problem.

You are given n pairs of strings. Two pairs (a,b) and (c,d) are identical if (a == c) and (c == d). That also implies (a,b) is not same as (b,a). 
...
public class JavaHashset {
 ...
 public static void main(String[] args) {
        ...
        for (int i = 0; i < t; i++) {
            elements.add(pair_left[i] + "," + pair_right[i]);
            ans[i] = elements.size();
            }
            for (int i = 0; i < t; i++) {
